...
def generate_room(maze, x, y, w, h, char="0", spawner_centers=None):
    # Заповнюємо кімнату
    ...
    # Додаємо спавнер у центр кімнати
    if spawner_centers is not None:
        center = get_center(x, y, w, h)
        maze[center[1]][center[0]] = "#"  # для візуалізації, не обов'язково
        spawner_centers.append(center)
        print("!! Room center:", center)

def generate_rooms(maze, maze_size, num_of_rooms, min_size, max_size, char, centers, occupied, spawner_centers=None):
    w, h = maze_size
    for _ in range(num_of_rooms):
        ...
        if not touching:
            generate_room(maze, x, y, room_w, room_h, char, spawner_centers)
...
def generate(size, num_of_rooms, min_size, max_size, num_of_lines, min_size_l, max_size_l):
    ...
    # Стартова кімната
    generate_room(maze, 1, 1, 3, 3, spawner_centers=spawner_centers)
    ...
    for char in ["0","2","3","4","5","6","7",]:
        generate_rooms(maze, size, num_of_rooms, min_size, max_size, char, centers, occupied, spawner_centers)
